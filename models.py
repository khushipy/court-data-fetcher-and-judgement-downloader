...
class CaseSearch(db.Model):
    ...
    court_type = db.Column(db.String(20), nullable=False)  # 'high_court' or 'district_court'
    search_date = db.Column(db.DateTime, default=datetime.utcnow)
    raw_response = db.Column(db.Text)
    
    # Relationship with CaseDetail (one-to-one)
    details = db.relationship('CaseDetail', backref='search', uselist=False)
...
class Party(db.Model):
    ...
    party_type = db.Column(db.String(50))  # 'Petitioner', 'Respondent', etc.
...
class CourtOrder(db.Model):
    ...
    order_type = db.Column(db.String(100))  # 'Interim Order', 'Final Judgment', etc.
